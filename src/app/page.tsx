  ...
  return (
    <div className="h-full flex font-mono text-white bg-black">
          ...
          {fields.map((field: string, index: number) => (
            <div className="w-full z-40" key={index}>
                {usedField !== index ? (
                  ...
                  <div>
                    <p className="flex justify-end pr-6 text-2xl h-14 items-center bg-white text-black border-b border-black border-r-2 rounded-r-lg transition-all" id={index.toString()} onClick={() => fieldFocus(index.toString())}>{field}</p>
                  </div>
                )}
            </div>
          ))}
      ...
      <div className="w-4/5 h-full">
        ...
        <Projects fields={fields} usedField={usedField}/>
        <Skills fields={fields} usedField={usedField}/>
        </div>
    </div>
  );
}